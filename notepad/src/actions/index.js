...
export const getNotes = () => {
    return (dispatch) => {
        ...
        axios
            .get(`http://localhost:5000/users/${user.id}/notes`)
            .then(response => {
...
export const getNote = (id) => {
    return (dispatch) => {
        dispatch({ type: PENDING });
        axios
            .get(`http://localhost:5000/users/${user.id}/notes/note/${id}`)
            .then(response => {
...
export const editNote = (note) => {
    return (dispatch) => {
        dispatch({ type: PENDING });
        axios
            .put(`http://localhost:5000/users/${user.id}/notes/note/${note.id}`, note)
            .then(response => {
                console.log('RES', response)
                dispatch({ type: SUCCESS_NOTE, note: response.data })
            })
...
export const createNote = (note) => {
    return dispatch => {
        ...
        console.log(note);
        axios
            .post(`http://localhost:5000/users/${user.id}/notes/new`, note)
            .then(response => {
                dispatch({ type: SUCCESS_NOTES, notes: response.data })
            })
            .catch(() => {
...
export const deleteNote = id => {
    return dispatch => {
        dispatch({ type: PENDING });
        axios
            .delete(`http://localhost:5000/users/${user.id}/notes/${id}`)
            .then(response => {
                dispatch({ type: SUCCESS_NOTES, notes: response.data })
            })
            .catch(err =>
