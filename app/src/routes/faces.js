...
router.post('/', async(req, res) => {
  try{
    let face = new Face(); // create a new instance of the Face model
    ...
    face.number_id = parseInt(req.body.number) - 1; // set the faces name (comes from the request)
...
router.get('/number/:number', async(req, res) => {
  try{
    ...
    for(var i = number; i < parseInt(number) + parseInt(config.faces_by_request); i++) {
