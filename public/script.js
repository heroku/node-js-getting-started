...
function highlight(el) {
    ...
    if (element.getAttribute("class") == "card text-white bg-warning mb-3") { remove2(el) };
...
function createOrderCardContent(responseObj) {
	...
	if(assignee == 'true') {assignee ="danger"};
	if(assignee == 'false') {assignee ="success" };
	if(assignee2 == 'true') {assignee2 ="danger"};
	...
	if(assignee2 == 'success' && assignee == 'success') result = "Done"
			...
			html2 = html2 + '<button id="b'+id+'" type="button" style="position: absolute;bottom: 0px;right: 1px;max-width: 80px;width: 25%;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;" class="btn btn-'+assignee2+'">Bar</button>' + '<button id="k'+id+'" type="button" style="position: absolute;bottom: 0px;left: 1px;max-width: 80px;width: 25%;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;" class="btn btn-'+assignee+'">Kitchen</button>'
